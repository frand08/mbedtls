/*
 *  Own certificate generation and signing
 *
...
#define DFL_SUBJECT_NAME "CN=Client,O=myorganization,C=AR"
#define DFL_ISSUER_NAME "CN=CA,O=myorganization,C=AR"
#define DFL_NOT_BEFORE "20010101000000"
#define DFL_NOT_AFTER "20311231235959"
#define DFL_SERIAL "1"
...
unsigned char client_cert[4096];
#define USAGE                                                                    \
  "\n usage: cert_write param=<>...\n"                                           \
  "\n acceptable parameters:\n" USAGE_CSR                                        \
  "    subject_key=%%s          default: subject.key\n"                          \
  "    subject_pwd=%%s          default: (empty)\n"                              \
  "    subject_name=%%s         default: CN=Client,O=myorganization,C=AR\n"      \
  "\n"                                                                           \
  "    issuer_crt=%%s           default: (empty)\n"                              \
  "                            If issuer_crt is specified, issuer_name is\n"     \
  "                            ignored!\n"                                       \
  "    issuer_name=%%s          default: CN=CA,O=myorganization,C=AR\n"          \
  "\n"                                                                           \
  ...
  "    not_after=%%s            default: 20311231235959\n"                       \
...
static uint8_t  md5[16] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 
                           0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF};
static uint32_t id = 0x1982;

/* FIXME: Create an entropy function based on ID */
static int own_entropy_func(void *data, unsigned char *output, size_t len) {
  int      ret;
  uint32_t counter = 0;
  if (data) {
    for (int i = 0; i < (int)len; i++) {
      counter += (id >> ((i) % 24)) & 0xFF;
      counter %= 16;
      output[i] = md5[counter] & (((id >> i) % 2) ? 0x0F : 0xF0);
    }
...
int own_ctr_drbg_random(void *p_rng, unsigned char *output, size_t output_len) {
  int                       ret;
...
int write_certificate(mbedtls_x509write_cert *crt, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng) {
  ...
  memset(client_cert, 0, 4096);
  if ((ret = mbedtls_x509write_crt_pem(crt, client_cert, 4096, f_rng, p_rng)) < 0) {
    return (ret);
  }
  return (0);
}

int certgen(options opt) {
  int ret       = 1;
  ...
  if ((ret = mbedtls_ctr_drbg_seed(
           &ctr_drbg, own_entropy_func, &entropy, (const unsigned char *)pers, strlen(pers))) != 0) {
  ...
  //
  if (!opt.selfsign && strlen((const char *)opt.issuer_crt)) {
    /*
     * 1.0.a. Load the certificates
     */
    mbedtls_printf("opt.issuer_crt size = %d\n", (int)strlen((const char *)opt.issuer_crt));
    mbedtls_printf("  . Loading the issuer certificate ...");
    fflush(stdout);

    if ((ret = mbedtls_x509_crt_parse_file(&issuer_crt, opt.issuer_crt)) != 0) {
      // if ((ret = mbedtls_x509_crt_parse(&issuer_crt, opt.issuer_crt, strlen((const char *)opt.issuer_crt))) != 0) {
      mbedtls_strerror(ret, buf, 1024);
      mbedtls_printf(
  ...
  if (!opt.selfsign && !strlen(opt.request_file)) {
    ...
    ret = mbedtls_pk_parse_keyfile(&loaded_subject_key, (const char *)opt.subject_key, opt.subject_pwd);
    if (ret != 0) {
      mbedtls_strerror(ret, buf, 1024);
      mbedtls_printf(
  ...
  ret = mbedtls_pk_parse_keyfile(&loaded_issuer_key, (const char *)opt.issuer_key, opt.issuer_pwd);
  if (ret != 0) {
    mbedtls_strerror(ret, buf, 1024);
    mbedtls_printf(
        " failed\n  !  mbedtls_pk_parse_keyfile "
  ...
  if (strlen((const char *)opt.issuer_crt)) {
  ...
  if (opt.version == MBEDTLS_X509_CRT_VERSION_3 && opt.basic_constraints != 0) {
  ...
  if (opt.version == MBEDTLS_X509_CRT_VERSION_3 && opt.subject_identifier != 0) {
  ...
  if (opt.version == MBEDTLS_X509_CRT_VERSION_3 && opt.authority_identifier != 0) {
  ...
  if ((ret = write_certificate(&crt, own_ctr_drbg_random, &ctr_drbg)) != 0) {
    mbedtls_strerror(ret, buf, 1024);
...
int main( int argc, char *argv[] )
{
    ...
    for( i = 1; i < argc; i++ )
    {
        ...
        *q++ = '\0';
        ...
        {
          id = (uint32_t)strtol((const char*)q, NULL, 16);
        }
        else if( strcmp( p, "secret_md5" ) == 0)
        {
          if (strlen(q) != 32)
          {
            ...
            for (int j = 0; j < 16; j++) {
              aux[0] = q[j*2];
              aux[1] = q[j*2+1];
              md5[j] = (uint8_t)strtol((const char*)&aux, NULL, 16);
            }
          }
        }
        else
    ...
    exit_code = certgen(opt);

    mbedtls_printf("output cert = \n%s", client_cert);

    len = strlen( (char *) client_cert );

    mbedtls_printf("SIZE = %d\n", (int)len);
    if( ( f = fopen( opt.output_file, "w" ) ) == NULL )
        return( -1 );

    if( fwrite( client_cert, 1, len, f ) != len )
    {
        fclose( f );
        return( -1 );
    }
